...
def add_forth_coord(points, value: float = 1):
    """forth coordinate is const = 1"""
    return np.hstack((points, value * np.ones((len(points), 1))))
...
def apply_transform(points: np.ndarray, transformation_list, apply_inverse: bool = False):
    """
    points = points х args[0] x args[1] x args[2] x ... args[-1]

    :param points: np.array N x (3|4)
    :param transformation_list: array of transformations. May be 4x4 np.arrays, or dict {
        'transformation': [t1, t2, t3],
        'rotation': [q1, q2, q3, q4],
        'scale': [s1, s2, s3],
    :param apply_inverse: should we inverse each transformation before application
    }
